...
global $hocwp_plugin, $plugin_page;

$plugin_name = $plugin_page;

if ( ! isset( $hocwp_plugin->{$plugin_name} ) ) {
	$plugin_name = str_replace( 'hocwp_', '', $plugin_name );
}
...
$obj = $hocwp_plugin->{$plugin_name};

if ( ! ( $obj instanceof HOCWP_Plugin_Core ) ) {
	return;
}

if ( ! current_user_can( $obj->capability ) || ( $obj->new_menu && ! current_user_can( 'manage_options' ) ) ) {
	return;
}
